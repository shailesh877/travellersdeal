...
const VendorDashboard = () => {
    ...
    const currencySymbol = { 'USD': '$', 'EUR': '€', 'GBP': '£', 'INR': '₹', 'AED': 'AED ', 'JPY': '¥' }[vendorCurrency] || (vendorCurrency ? `${vendorCurrency} ` : '');
                                            ...
                                            {bookings.slice(0, 5).map((booking) => (
                                                <tr key={booking._id} className="hover:bg-white transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 font-bold text-gray-900">
                                                        {{ 'USD': '$', 'EUR': '€', 'GBP': '£', 'INR': '₹', 'AED': 'AED ', 'JPY': '¥' }[booking.experience?.bookingOptions?.[0]?.availabilityAndPricing?.currency || booking.experience?.currency] || ((booking.experience?.bookingOptions?.[0]?.availabilityAndPricing?.currency || booking.experience?.currency) ? `${(booking.experience?.bookingOptions?.[0]?.availabilityAndPricing?.currency || booking.experience?.currency)} ` : '')}{booking.totalPrice}
                                                    </td>
                                                    <td className="px-6 py-4">
